...
test.describe('Donations - Mes donations', () => {
  ...
  test('devrait afficher la liste des donations ou l\'état vide', async ({ page }) => {
    await page.goto('/my-donations');

    // Liste ou état vide selon les données
    await expect(
      page.getByTestId('my-donations-list')
        .or(page.getByTestId('my-donations-empty'))
    ).toBeVisible();
  });
  ...
  test('devrait afficher les cartes de donation avec les informations', async ({ page }) => {
    ...
    const donationCards = page.getByTestId('donation-card-compact');
  ...
  test('devrait afficher l\'état vide ou la liste des donations', async ({ page }) => {
    await page.goto('/my-donations');

    // Attendre que le contenu soit chargé (liste ou état vide)
    await expect(
      page.getByTestId('my-donations-list')
        .or(page.getByTestId('my-donations-empty'))
    ).toBeVisible();
